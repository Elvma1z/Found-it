...
class _CollapsibleDockTitle(QWidget):
    ...
    is what made their button rows overlap). Dragging/floating the dock
    still works from the rest of the bar since only the arrow eats clicks."""
    ...
    def apply_theme(self, p: dict):
        ...
        self.toggle_btn.setStyleSheet(f"""
            QPushButton {{ color: {p['text']}; border: none; background: transparent; font-size: 11px; }}
...
class RoomSetupPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Zones/Drawers/Detected Objects are independent dock panels - drag
        # by their title bar to move or tab them together, drag an edge to
        # resize, or float/close them (reopen via the "Panels" menu).
        # Exposed as _room_docks so Panel Customization can still pull each
        # one out to the sidebar individually.
